...
def get_weather_data(county_name):
    ...
    try:
        ...
        st.warning(f"⚠️ 天氣資料連線失敗，將使用預設值。({e})")
        return None
...
def get_master_log():
    try: 
        return conn.read(worksheet="叫貨紀錄", ttl="1m")
    except Exception as e:
        st.warning(f"⚠️ 無法讀取叫貨紀錄：{e}")
        return None
...
with tab_order:
    try:
        ...
        if '到貨天數' in vendor_info and pd.notna(vendor_info['到貨天數']):
            try: 
                lead_time = int(vendor_info['到貨天數'])
            except ValueError: 
                pass
                ...
                if order_rows:
                    for sheet in ["叫貨紀錄", f"紀錄_{selected_vendor}"]:
                        try:
                            ...
                            st.warning(f"⚠️ 更新資料表 {sheet} 時發生錯誤: {e}")
        ...
        if hist_df is not None and not hist_df.empty:
            ...
            if not hist_df.empty:
                ...
                with c1:
                    ...
                    hist_df = hist_df.drop(columns=['純日期']) 
